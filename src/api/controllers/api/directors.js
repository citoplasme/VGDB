...
const Directors = module.exports

Directors.list = () => {
    const query = `select distinct ?id ?name where{
        ?g rdf:type games:Director .
        ?g games:name ?name .
...
Directors.findOne = async (id) => {
    const query = `select ?id ?name where{
        games:${id} rdf:type games:Director .
...
Directors.findGames = async (id) => {
    const query = `select ?id ?name where{
        games:${id} rdf:type games:Director .
        ?i rdf:type games:VideoGame .
    	?i games:directedby games:${id} .
    	?i games:title ?name .
