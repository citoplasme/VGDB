<template>
  <div>
    <Loading v-if="!gamesReady" :message="'developers'" />
...
import GameListing from "@/components/developers/DeveloperListing.vue";
...
export default {
  ...
  methods: {
    ...
    goEditar(id) {
      this.$router.push("/developers/editar/" + id);
    },

    remover(id) {
      this.$request("delete", "/developers/" + id)
        .then(res => {
          this.text = res.data;
          this.color = "success";
          this.snackbar = true;
    ...
    async getGenres() {
      try {
        let response = await this.$request("get", "/developers");
