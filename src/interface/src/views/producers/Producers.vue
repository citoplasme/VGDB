<template>
  <div>
    <Loading v-if="!gamesReady" :message="'producers'" />
...
import GameListing from "@/components/producers/ProducerListing.vue";
...
export default {
  ...
  methods: {
    ...
    go(id) {
      this.$router.push("/producers/" + id);
    },

    goEditar(id) {
      this.$router.push("/producers/editar/" + id);
    },

    remover(id) {
      this.$request("delete", "/producers/" + id)
        .then(res => {
          this.text = res.data;
          this.color = "success";
          this.snackbar = true;
    ...
    fecharSnackbar() {
      this.snackbar = false;
      if (this.done == true) this.getGenres();
    },

    async getGenres() {
      try {
        let response = await this.$request("get", "/producers");
