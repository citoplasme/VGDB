<template>
  <div>
    <Loading v-if="!gamesReady" :message="'age ratings'" />
...
import GameListing from "@/components/ratings/RatingsListing.vue";
...
export default {
  ...
  methods: {
    ...
    goEditar(id) {
      this.$router.push("/ratings/editar/" + id);
    },

    remover(id) {
      this.$request("delete", "/ratings/" + id)
        .then(res => {
          this.text = res.data;
          this.color = "success";
          this.snackbar = true;
    ...
    async getGenres() {
      try {
        let response = await this.$request("get", "/ratings");
