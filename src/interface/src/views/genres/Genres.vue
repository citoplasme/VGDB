<template>
  <div>
    <Loading v-if="!gamesReady" :message="'genres'" />
...
import GameListing from "@/components/genres/GenreListing.vue";
...
export default {
  ...
  methods: {
    ...
    goEditar(id) {
      this.$router.push("/genres/editar/" + id);
    },

    remover(id) {
      this.$request("delete", "/genres/" + id)
        .then(res => {
          this.text = res.data;
          this.color = "success";
          this.snackbar = true;
    ...
    async getGenres() {
      try {
        let response = await this.$request("get", "/genres");
