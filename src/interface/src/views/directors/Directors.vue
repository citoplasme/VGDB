<template>
  <div>
    <Loading v-if="!gamesReady" :message="'directors'" />
...
import GameListing from "@/components/directors/DirectorListing.vue";
...
export default {
  ...
  methods: {
    ...
    go(id) {
      this.$router.push("/directors/" + id);
    },

    goEditar(id) {
      this.$router.push("/directors/editar/" + id);
    },

    remover(id) {
      this.$request("delete", "/directors/" + id)
        .then(res => {
          this.text = res.data;
          this.color = "success";
          this.snackbar = true;
    ...
    async getGenres() {
      try {
        let response = await this.$request("get", "/directors");
