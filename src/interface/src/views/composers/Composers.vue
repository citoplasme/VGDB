<template>
  <div>
    <Loading v-if="!gamesReady" :message="'composers'" />
...
import GameListing from "@/components/composers/ComposerListing.vue";
...
export default {
  ...
  methods: {
    ...
    go(id) {
      this.$router.push("/composers/" + id);
    },

    goEditar(id) {
      this.$router.push("/composers/editar/" + id);
    },

    remover(id) {
      this.$request("delete", "/composers/" + id)
        .then(res => {
          this.text = res.data;
          this.color = "success";
          this.snackbar = true;
    ...
    async getGenres() {
      try {
        let response = await this.$request("get", "/composers");
