<template>
  <div>
    <Loading v-if="!gamesReady" :message="'game modes'" />
...
import GameListing from "@/components/modes/ModeListing.vue";
...
export default {
  ...
  methods: {
    ...
    go(id) {
      this.$router.push("/modes/" + id);
    },

    goEditar(id) {
      this.$router.push("/modes/editar/" + id);
    },

    remover(id) {
      this.$request("delete", "/modes/" + id)
        .then(res => {
          this.text = res.data;
          this.color = "success";
          this.snackbar = true;
    ...
    async getGenres() {
      try {
        let response = await this.$request("get", "/modes");
