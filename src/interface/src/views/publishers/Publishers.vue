<template>
  <div>
    <Loading v-if="!gamesReady" :message="'publishers'" />
...
import GameListing from "@/components/publishers/PublisherListing.vue";
...
export default {
  ...
  methods: {
    ...
    goEditar(id) {
      this.$router.push("/publishers/editar/" + id);
    },

    remover(id) {
      this.$request("delete", "/publishers/" + id)
        .then(res => {
          this.text = res.data;
          this.color = "success";
          this.snackbar = true;
    ...
    fecharSnackbar() {
      this.snackbar = false;
      if (this.done == true) this.getPublishers();
    },

    async getPublishers() {
      try {
        let response = await this.$request("get", "/publishers");
