<template>
  <div>
    <Loading v-if="!gamesReady" :message="'platforms'" />
...
import GameListing from "@/components/platforms/PlatformListing.vue";
...
export default {
  ...
  methods: {
    ...
    goEditar(id) {
      this.$router.push("/platforms/editar/" + id);
    },

    remover(id) {
      this.$request("delete", "/platforms/" + id)
        .then(res => {
          this.text = res.data;
          this.color = "success";
          this.snackbar = true;
    ...
    async getPlatforms() {
      try {
        let response = await this.$request("get", "/platforms");
