<template>
  <div>
    <Loading v-if="!gamesReady" :message="'entities'" />
...
import GameListing from "@/components/entities/EntityListing.vue";
...
export default {
  ...
  methods: {
    ...
    goEditar(id) {
      this.$router.push("/entities/editar/" + id);
    },

    remover(id) {
      this.$request("delete", "/entities/" + id)
        .then(res => {
          this.text = res.data;
          this.color = "success";
          this.snackbar = true;
    ...
    async getGenres() {
      try {
        let response = await this.$request("get", "/entities");
