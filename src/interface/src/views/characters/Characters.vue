<template>
  <div>
    <Loading v-if="!gamesReady" :message="'characters'" />
...
import GameListing from "@/components/characters/CharacterListing.vue";
...
export default {
  ...
  methods: {
    ...
    goEditar(id) {
      this.$router.push("/characters/editar/" + id);
    },

    remover(id) {
      this.$request("delete", "/characters/" + id)
        .then(res => {
          this.text = res.data;
          this.color = "success";
          this.snackbar = true;
    ...
    async getGenres() {
      try {
        let response = await this.$request("get", "/characters");
