<template>
  <div>
    <Loading v-if="!gamesReady" :message="'countries'" />
...
import GameListing from "@/components/countries/CountriesListing.vue";
...
export default {
  ...
  methods: {
    ...
    goEditar(id) {
      this.$router.push("/countries/editar/" + id);
    },

    remover(id) {
      this.$request("delete", "/countries/" + id)
        .then(res => {
          this.text = res.data;
          this.color = "success";
          this.snackbar = true;
    ...
    async getGenres() {
      try {
        let response = await this.$request("get", "/countries");
