<template>
  <div>
    <Loading v-if="!gamesReady" :message="'designers'" />
...
import GameListing from "@/components/designers/DesignerListing.vue";
...
export default {
  ...
  methods: {
    ...
    goEditar(id) {
      this.$router.push("/designers/editar/" + id);
    },

    remover(id) {
      this.$request("delete", "/designers/" + id)
        .then(res => {
          this.text = res.data;
          this.color = "success";
          this.snackbar = true;
    ...
    async getGenres() {
      try {
        let response = await this.$request("get", "/designers");
