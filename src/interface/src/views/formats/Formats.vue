<template>
  <div>
    <Loading v-if="!gamesReady" :message="'formats'" />
...
import GameListing from "@/components/formats/FormatListing.vue";
...
export default {
  ...
  methods: {
    ...
    go(id) {
      this.$router.push("/formats/" + id);
    },

    goEditar(id) {
      this.$router.push("/formats/editar/" + id);
    },

    remover(id) {
      this.$request("delete", "/formats/" + id)
        .then(res => {
          this.text = res.data;
          this.color = "success";
          this.snackbar = true;
    ...
    async getGenres() {
      try {
        let response = await this.$request("get", "/formats");
