<template>
  <div>
    <Loading v-if="!gamesReady" :message="'your favorites'" />
    <v-card v-else class="ma-8">
      ...
      <v-dialog :value="eliminarId != ''" persistent max-width="290px">
        <v-card>
          ...
          <v-card-text>
            Are you sure you want to remove the game from the list?
          </v-card-text>
          <v-card-actions>
            <v-spacer></v-spacer>
...
import GameListing from "@/components/favorites/FavoriteListing.vue";
import { NIVEL_MINIMO_ALTERAR } from "@/utils/consts";

export default {
  data: () => ({
    tipo: "Favorite Video Games",
    games: [],
    campos: [],
    cabecalhos: [],
    operacoes: [{ icon: "delete", descricao: "Remoção", cor: "red" }],
    gamesReady: false,
    search: "",
    headers: [],
    dialog: false,
  ...
  methods: {
    ...
    preparaCabecalhos() {
      ...
      this.campos = ["id", "title", "operacoes"];
    ...
    remover(id) {
      this.$request("delete", "/users/favorites/" + id)
        .then(res => {
          this.text = res.data;
          this.color = "success";
          this.snackbar = true;
    ...
    async getFavorites() {
      try {
        let response = await this.$request("get", "/users/favorites");

        this.games = await this.preparaLista(response.data);
